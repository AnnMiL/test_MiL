...
%% load data
[op] = pathoptiset(mfilename('fullpath'), 'o','d','ARM3and4stagesround2Step2.mat');
load(op.fpath);
[Temall, config] = getTemfromResults(resultData);


step = config(1).step;
name=[str2, str4, 'step',num2str(step),'.eps'];
tem = Temall(:,4:end)';
%tem = tem - 345;  % data to plot
...
set(AH(1),'xlim',[0.5 3.5],'xtick',1:1:3,'ylim',[345 380],'ytick',345:10:390,...
