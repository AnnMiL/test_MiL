...
%% load data
[op] = pathoptiset(mfilename('fullpath'), 'o','d','IntelSCC3and4stagesround2Step2.mat');
load(op.fpath);
[Temall, config] = getTemfromResults(resultData);


step = config(1).step;
name=[str2, str6, 'step',num2str(step),'.eps'];
tem = Temall(:,4:end)';
...
set(AH(1),'xlim',[0.5 3.5],'xtick',1:1:3,'ylim',[329 335],'ytick',329:1:335,...
...
%% new labels
xtextp=xt-0.1;
ytextp=(yt(1)-0.2*(yt(2)-yt(1)))*ones(1,length(xt))-0.3;
